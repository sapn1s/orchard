```orchard-ticket
{
  "id": "ARCH-001",
  "type": "architecture",
  "title": "Liveness decisions now share one server authority",
  "summary": "Running and ended states now come from one server authority, and the client renders its snapshot. Conformance and regression suites passed across survivor, restart, reload, agent, refusal, and zombie cases. The enforcement guard still covers only server code, so new client-side checks are not mechanically prevented.",
  "impact_if_we_wait": "The remaining client-side guard gap could let future interface code derive liveness independently and reintroduce conflicting displays. Bounded: this concerns display-correctness and refusal behavior, not stored data loss; current server decisions and snapshot rendering are already centralized.",
  "current_need": "Treat the architecture work as closed: targeted liveness regressions passed, broad pass tallies matched, and type checking stayed clean.",
  "severity": "high",
  "area": "Session and agent liveness",
  "reported": "2026-08-13",
  ...
  "decision_history": [
    {
      "asked_on": "2026-08-09",
      "question": "How should the project establish one authoritative answer for whether work is alive?",
      "mode": "single",
      "options_keys": [
        "1",
        "2",
      ...
      "chosen_by": "user",
      "note": "The user chose one server-side liveness service, sequenced with the server-authored client snapshot and a conformance test written first."
    }
  ],
  "success_criteria": [
    "Every server-side liveness decision consults the shared authority",
    "The client renders the server-authored liveness snapshot",
    "All covered sites agree across the conformance matrix",
    "Stale survivors never block sends while genuinely busy survivors always do",
    "New server-side liveness checks cannot bypass the shared service"
  ],
  "code_refs": [
    {
      "path": "src/server/**",
      "symbol": null,
      "note": "The guard prevents new server-side liveness checks but does not cover client code."
    },
    {
      "path": "watcher.ts",
      "symbol": null,
      "note": "Previously inferred liveness from a 30-second modification-time window."
    },
    {
      "path": "app.js",
      "symbol": null,
      "note": "Previously combined event fragments with a live-bridge override instead of rendering server-authored truth."
    }
  ],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/ARCH-001-single-liveness-authority.md",
    "sha256": "74bb22086ac355a19b3ed82c0b433a588ec7a6f61031d495b2c0f807ecefa41e",
    "bytes": 26006,
    "original_title": "no single authority answers \"is this thing alive?\", so every call site re-decides",
    "migrated_on": "2026-08-19",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the supplied original; its diagnosis, chosen architecture, migration sequence, proof bar, executed evidence, and client-side guard gap are preserved.",
    "dropped": []
  }
}
```

# ARCH-001 — Liveness decisions now share one server authority

## Diagnosis

ARCH-001 identified several independent answers to whether a session or agent was alive or mid-turn. Bridge state, survivor records, modification times, and client event fragments could disagree because each surface interpreted partial signals locally. This produced phantom or missing rows, stuck activity indicators, zombie-busy states, false timers, and refusals based on stale state.

## Evidence

The fixer ran `verify:health-survivor` with 14/14 passing, `verify:hosts-cleanup` with 17/17, `verify:restart-survives` with 15/15, `verify:restart-reconnect-race` with 10/10, `verify:reload-live-summary` with 10/10, `verify:agent-summary` with 3/3, `verify:detach` with 7/7, `verify:stale-agent-cards` with 12/12, `verify:refusal-visible` with 16/16, and `verify:zombie-busy` with 34/34. Additional recorded pass tallies were 5/5, 191/191, 77/77, 4/4, 9/9, 476/476, 96/96, and 47/47. Type checking was clean. `verify:liveness-conformance`, `verify:resume-refusal`, `verify:live`, and `verify:ui` were named without recorded results.

## Implementation notes

Phase 1 introduced the server-side authority and conformance gate. Server decisions for reattachment, refusal, reaping, health, and cleanup were moved toward the shared answer. Phase 2 made the client render the server-authored snapshot, with BUG-034 covering snapshot rendering and FEAT-057 supplying the ended state. The guard against bypassing the authority applies under `src/server/**` only.

## Verification plan

Exercise survivor cleanup, host cleanup, restart survival, reconnect races, reload summaries, agent summaries, detachment, stale cards, refusal visibility, and zombie-busy behavior. Keep type checking clean. For future changes, run the conformance matrix and test both stale-survivor and genuinely-busy send decisions.

## Migration and rollback

The service was introduced beside existing checks, then call sites were converted incrementally while their focused suites stayed green. The client subsequently switched to server-authored snapshots. Each conversion can be reverted separately if a call site regresses, while retaining the shared service for converted sites.

## Risks

The remaining enforcement gap is client-side: a future client check could derive liveness again without being rejected by the server-directory guard. Incremental conversion also risked temporary disagreement with in-flight work, which is why focused suites accompanied each converted path.
