```orchard-ticket
{
  "id": "ARCH-004",
  "type": "architecture",
  "title": "Open tickets silently disappear from the board",
  "summary": "Tickets awaiting action can appear finished, unassigned, or absent from attention views without warning. State is inferred separately from status prose, ownership, and table placement. Existing pattern fixes address individual cases but leave these representations able to disagree.",
  "impact_if_we_wait": "Open work can remain invisible until someone notices an incorrect count. Bounded: this affects ticket discoverability and display-correctness, not ticket contents or user data. Manual flag corrections address known cases but not recurrence.",
  "current_need": "An independent clean-room pass on round 7, which is the only round self-verified so far; the canonical state itself is built and every board tool reads it.",
  "severity": "high",
  "area": "Ticket board",
  "reported": "2026-08-18",
  "reported_by": "user",
  ...
  "decision_history": [
    {
      "asked_on": "2026-08-18",
      "question": "How should the board prevent open tickets from silently becoming unreachable?",
      "mode": "single",
      "options_keys": [
        "A",
        "B",
      ...
      "chosen_by": "user",
      "note": "Answered through the ticket view on 2026-08-18 — a fixed set of states, not prose. Option B was built the next day and has been through seven rounds; option C's reachability check was kept as a ride-along rather than an alternative. The record carried the answer only as a log entry until 2026-08-25, when the ARCH-010 class decision found this ticket still declaring itself undecided."
    }
  ],
  "success_criteria": [
    "Each ticket has one machine-readable state selected from a fixed set.",
    "Board placement and human-attention markers derive from the canonical state.",
    "Every open ticket appears on at least one human-visible board surface.",
    "The checker reports any open ticket that appears on no human-visible surface.",
    "Incidental words in ticket prose cannot change state or board placement."
  ],
  "code_refs": [
    {
      "path": "scripts/board.mjs",
      "symbol": "isDoneStatus",
      "note": "Matches DONE in status prose to determine open or finished placement."
    },
    {
      "path": "docs/bugs/INDEX.md",
      "symbol": null,
      "note": "Contains the manually curated owner cells used for human-attention state."
    },
    {
      "path": "src/server/board.ts",
      "symbol": "readBoard",
      "note": "Reads board representations and derives attention lanes."
    },
    {
      "path": "src/server/board.ts",
      "symbol": "boardSummary",
      "note": "Computes derived board lanes from ownership, status, and activity information."
    },
    {
      "path": "src/server/board.ts",
      "symbol": "alreadyAnswered",
      "note": "Previously matched marker text across the ticket and could hide tickets from the attention rail."
    }
  ],
  "related": [
    {
  ...
  "verification": [
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a019a4-bda2-7101-935b-80e381544e05",
      "verdict": "broken",
    ...
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a019c1-70fd-7100-a0b1-a5c8874c6c2f",
      "verdict": "broken",
      "verdict_on": "2026-08-19",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a019d8-d91f-7c13-bbaf-29c4497d9455",
      "verdict": "broken",
      "verdict_on": "2026-08-19",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a019f8-d059-74c3-9f60-a31a1cfcf9e6",
      "verdict": "broken",
      "verdict_on": "2026-08-19",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a01a13-3561-7b80-80a7-e9f5504aca7c",
      "verdict": "broken",
      "verdict_on": "2026-08-19",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a01a30-e639-7300-9c29-78aa480976ac",
      "verdict": "broken",
      "verdict_on": "2026-08-19",
      "harness": "scripts/independent-verify.mjs"
    }
  ...
  "source": {
    "archived_path": "docs/bugs/archive/ARCH-004-ticket-state-lives-in-prose-so-tickets-go-missing.md",
    "sha256": "9819e139d510be570b9c9cbf8fef7d52ab8c5d137cca4ea365877dd72ebaf17d",
    "bytes": 68613,
    "original_title": "a ticket's state lives in prose, so tickets silently disappear and nothing notices",
    "migrated_on": "2026-08-19",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the supplied ticket text; the three incidents, split-state diagnosis, live options, recommendation, migration condition, reachability requirement, and bounds are preserved.",
    "dropped": []
  }
}
```

# ARCH-004 — Open tickets silently disappear from the board

## Diagnosis

Ticket state currently has three independent representations: prose status, a manually maintained owner marker, and board placement inferred through pattern matching. None is authoritative, so they can disagree without producing a detectable contradiction.

The finished-table parser treats an incidental `DONE` as deliberate state. Human-attention membership instead follows the owner cell. Attention lanes also combine these values with activity-derived markers. Repairing individual patterns removes one trigger while preserving the underlying split state.

## Evidence

Seven tickets containing recommendations were not marked as needing the user, so a filter returned one instead of seven. `FEAT-082` entered the finished table because its status contained “proposal done.” Another ticket disappeared from the attention rail when descriptive prose matched an answered marker.

The existing `board:check` remained clean because it compares representations for agreement, not whether an open ticket is reachable through a human-visible surface.

`verify:reachability` ran with 17/17 passing, and `verify:decision-shape` ran with 25/25 passing. Additional recorded pass tallies were 16/16, 4/4, and 185/185 without adjacent suite names. Type checking and `board:check` were reported clean.

## Implementation notes

For option B, add one fixed machine-readable state to each ticket and make it authoritative. Derive board placement and the human-attention marker from that field. Retain the prose status as explanatory text only.

Add an invariant that every open ticket appears on at least one surface a person uses. Report any open ticket with no reachable surface. Do not treat agreement between the index and ticket files as sufficient evidence of reachability.

For option C, add only the reachability invariant and preserve the current prose-derived model.

## Verification plan

Create cases where status prose contains incidental state words, recommendation text appears without an owner marker, and an answered phrase appears outside its structural marker. Confirm these words cannot silently remove an open ticket from all human-visible surfaces.

For the canonical-state option, confirm placement and attention markers derive only from the fixed state. For either viable checking option, confirm an unreachable open ticket fails the reachability suite while correctly surfaced open tickets pass.

Re-run type checking, board consistency checks, ticket schema checks, provenance checks, board tooling checks, and the affected board and attention suites.

## Migration and rollback

Inventory existing status and owner combinations before migration. Translate each ticket into the fixed state, report ambiguous combinations for review, then switch placement and attention derivation to the canonical field.

Land the state field, migration, derivation changes, and reachability check as separable steps. Roll back derivation independently while retaining migrated fields and the reachability check if board behavior regresses.

## Risks

A migration can encode existing incorrect placement as canonical state unless ambiguous records receive review. Deriving every surface from one field reduces drift but makes a bad state value propagate consistently. A reachability-only check detects disappearance but leaves repeated pattern collisions and manual correction work.
...
### 2026-08-25 — the ticket that fixed derived state was itself declaring a state nobody had updated

`regressed-from: nothing — this is a record-hygiene correction, not a code change.`

- **Understood:** the ARCH-010 class decision (option A, 2026-08-25) required re-establishing which
  instances of the class are still standing. This one is built: the user answered it on 2026-08-18
  ("obviously it must be fixed set choices" = option B), option B landed the next day, and seven
  rounds have hardened it. But its own record still said `work_state: open`, `human_action: decide`,
  `owner: you`, with the live four-option decision still in place — so every reader of this board
  has been told for a week that the ticket which ended derived ticket state is waiting for someone
  to decide it. The answer existed only as prose in the activity log, which is precisely the defect
  this ticket exists to end, applied to itself.
- **Changed:** this ticket's record only. The decision moved to `decision_history` (B, chosen by the
  user on 2026-08-18); `human_action` → `none`; `owner` → unassigned; `work_state` → `in_verification`,
  which is the honest state — round 6's clean room returned BROKEN, round 7 fixed it and **round 7
  has only ever been verified by the lane that wrote it**; `current_need` now names that pass. No
  prose above was edited, no log entry altered, no code, no scripts.
- **Verified:** `validateTicket` ok; `npm run board:check` exit 0, read directly. This lane changed
  no behaviour, so there is no must-fail to show. What is falsifiable is the state claim itself: the
  round-7 entry above records `verify:unmappable-status` 692/0 and a two-room placement audit, and
  no `Verified-by:` line for round 7 — which is why the state is `in_verification` and not
  `verified`.
- **Still open / handoff:** an independent clean-room pass on round 7, with the attack this ticket's
  own round-7 entry names (does the homogeneous-run requirement now miss a line a human plainly
  reads as a declaration?). Nothing else. This ticket is **not** closed by the class decision — it
  is the one instance where the class was already settled and only the board did not know.

### 2026-09-23 — the last incidental-DONE sweep is REMOVED, not merely flagged (ARCH-017 was live proof)

`regressed-from: 15759b0` — the one-definition-of-done commit that introduced `classifyLegacyStatus`
KEPT board.mjs's match-anywhere `DONE` and only made the resulting misplacement *observable*
(`ambiguous: true` + an `AMBIGUOUS STATUS` advisory). "Observable" was not enough: the ticket still
went into Done.

**The defect, measured on the real board.** `classifyLegacyStatus` matched a bare `DONE` ANYWHERE in
the status line — so a legacy ticket whose leading declared word is NOT done but whose header merely
*mentions* the word (a quoted verdict, a proof bar, prose) was swept to Done. **ARCH-017** was the live
instance: leading `IN PROGRESS`, with `…was reported DONE with a happy-path screenshot and WAS NOT` deep
in a 12 KB round-history header, classified `done: true` and the board listed it under **Done** while its
own record and every fix lane read it as IN PROGRESS. A user and an orchestrator spent a long exchange
disagreeing about the ticket's state for exactly this reason. Blast radius, measured over all 319 ticket
files: **exactly ONE** legacy ticket fired the incidental-DONE sweep — ARCH-017. (FEAT-020 "SYNTHESIS …
DONE" and FEAT-043, the two tickets prior rounds named, are now block-format and read `work_state`
directly, so they never reach this path.)

**The fix (ARCH-010: the owner declares the fact; no reader re-derives it).** A legacy ticket's state is
now the DECLARED leading word only, anchored at the start of the `- **Status:**` line (every
`LEGACY_STATUS_TABLE` `re` already begins `^`). Both match-anywhere `DONE` branches in
`classifyLegacyStatus` are DELETED — there is no path left by which a word later in the line can override
the leading declaration. A block ticket never reached this code (`blockSummary` reads `work_state`
directly), so the one place a legacy prose state is derived is now anchored, and the `ambiguous` outcome
no longer exists. No second parser was added; `board:gen`/`board:check`, `tickets.ts` and `arch-watch`
all consume the same function.

**A second, latent board defect this surfaced and I fixed with it.** Moving ARCH-017 into the Open table
(it had lived in Done, which has no Status column) put its pipe-bearing status (`stamp || waited`) into a
`|`-delimited cell. `board.mjs` WRITES a literal `|` as `\|` ("so it cannot split the row") but
`indexRowCells` split on every raw `|` and never un-escaped — so it read a spurious extra column and
`board:check` reported a false `SEVERITY MISMATCH: ARCH-017`. `indexRowCells` now splits on UNESCAPED
pipes and un-escapes `\|`→`|`, matching the writer. Only one INDEX row (ARCH-017's, just generated) has
an escaped pipe today, so nothing else moves; ordinary rows are byte-for-byte unaffected.

**Files.** `scripts/lib/ticket-schema.mjs` (`classifyLegacyStatus` + `indexRowCells` + doc comments),
`scripts/board.mjs` (comment only), `scripts/verify-ticket-schema.mjs` and
`scripts/verify-unmappable-status.mjs` (the assertions that PINNED the old incidental-DONE behaviour now
assert the new one), and `docs/bugs/INDEX.md` (regenerated by `board:gen` — ARCH-017 Done→Open, the only
row that moved).

**Verification (executed output, WA §C).**
- (i) ARCH-017 now classifies OPEN: `parseTicket` → `work_state: in_progress`, `done: false`,
  `matched: true`, `ambiguous: false`, no error/warning. `board:gen` moved it Done→Open (INDEX diff:
  +4/−1, ARCH-017 the only id whose row changed); `board:check` then dropped its `STATUS MISMATCH:
  ARCH-017 … (⇒ Open) but board lists it in Done`.
- (ii) CONTROL — a genuinely Done ticket stays Done: BUG-004 (block, `work_state: verified`) →
  `done: true`, still in the Done table. The done-set was NOT widened.
- (iii) NON-VACUITY (synthesised, real-shape): with the SAME `…reported DONE … WAS NOT` prose, only the
  leading word changed — `IN PROGRESS — …DONE…` ⇒ `in_progress`/open; `VERIFIED — …DONE…` ⇒
  `verified`/done; `SYNTHESIS — …DONE…` (no leading word) ⇒ UNMAPPABLE/open, not swept to Done. Flipping
  the declared word flips the result, so the check cannot pass on missing input.
- (iv) TICKETS WHOSE CLASSIFICATION CHANGES: **ARCH-017 only** (Done→Open — leading `IN PROGRESS` now
  wins over the incidental `DONE`). No other ticket, in either direction (measured across all 319 files).
- `board:check`: exit 1 with exactly the **5 pre-existing** drifts this ticket does not own and ARCH-017's
  own round-11 header already names — `UNMAPPABLE STATUS` BUG-169/FEAT-129/FEAT-131/FEAT-132 (genuinely
  unmapped status words: REOPENED / OPTION A / IMPLEMENTED) and `UNREACHABLE TICKET` ARCH-007. Zero new
  drift from this change. `board:gen` is idempotent (re-run produces no diff).
- `npm run gate`: **PASS, exit 0** (leak-gate, check-nul, typecheck), read unpiped.
- Anti-regression: `verify:ticket-schema` 62/2 — the 2 failures are the SAME pre-existing 4-undetermined
  tickets, confirmed present on HEAD via HEAD's own module (not mine). `verify:unmappable-status` 674/18
  vs HEAD 673/19 (one fewer failure, none new) — the ~18 are pre-existing/environmental (the scaffold
  copy + HEAD-comparison machinery does not run in this sandbox: `node:esm/resolve` errors, `head=0`,
  `headGen exit=1`; plus the 4 unmapped tickets and the round-6 quoted-example cases). `verify:board-tool`
  27/7 vs HEAD 7-failing — same "check-clean-over-the-real-corpus" family that fails because the scratch
  board copies the 5 drifted real tickets; not this change. The incidental-DONE unit cases I edited PASS.
- **Could not test:** the live HTTP/browser render of the Open-table row (exercised `parseTicket` /
  `board.mjs` in-process); no scratch server was booted for this classifier-only change.

**Still open / handoff (unchanged by this lane):** the round-7 parser-collapse still wants an independent
clean-room pass. This lane's change is small and mechanical (delete two branches + one escape fix) but
touches the same regression-prone parser, so an independent clean-room verify of THIS change is warranted
before ARCH-004 is called VERIFIED — generation must not be its own only verifier.
