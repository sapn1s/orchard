```orchard-ticket
{
  "id": "BUG-034",
  "type": "bug",
  "title": "Main session work now appears in the agents strip",
  "summary": "The agents strip now represents active main-session work using a server-authored running snapshot. Previously, responses streamed while the strip appeared idle. The implemented design uses a main row rather than relying only on the crown indicator.",
  "impact_if_we_wait": "The old display concealed active main-session work and could mislead users about whether processing continued. Bounded: this affected live-state display correctness, not message delivery, session execution, or stored data.",
  "current_need": "Treat the ticket as closed: the liveness and agent-state suites passed, and the standing type check stayed clean.",
  "severity": "medium",
  "area": "Agents strip live state",
  "reported": "2026-08-09",
  "reported_by": "user",
  "owner": "agent",
  "work_state": "verified",
  ...
  "decision_history": [
    {
      "asked_on": "2026-08-09",
      "question": "Should main-session work appear in the agents strip or only through the crown indicator?",
      "mode": "single",
      "options_keys": [
        "A",
        "B"
      ...
      "chosen_by": "agent",
      "note": "The strip now renders the server-authored running set, including active main-session work."
    }
  ],
  "success_criteria": [
    "Active main-session work appears on the agreed live-state surface",
    "The main-session indicator settles when the turn ends",
    "A killed session does not remain indefinitely marked as running",
    "Main-session elapsed time is derived from genuine live work",
    "Subagent backfill and settling behavior remain intact"
  ],
  "code_refs": [],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/BUG-034-main-agent-row-missing.md",
    "sha256": "a2080d3ce2cf5a0caace6da0abf9ff0e081eea66cf4dd1123d1c41092b3a87b9",
    "bytes": 18032,
    "original_title": "agents strip shows no row for the MAIN session while it's working",
    "migrated_on": "2026-08-19",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the supplied ticket text; the symptom, design choice, live-state invariants, regression coverage, and recorded execution evidence remain represented.",
    "dropped": []
  }
}
```

# BUG-034 — Main session work now appears in the agents strip

## Diagnosis

The agents strip could remain empty while the main session was thinking, streaming, or running tools. A main row had appeared in stale or reattached cases, so the ticket required distinguishing those reconstructed states from the normal turn lifecycle. The implemented design replaced that ambiguity with a server-authored running-set snapshot.

## Evidence

The liveness conformance suite passed 96/96. Stale agent cards passed 12/12, reattach backfill passed 15/15, agent summaries passed 3/3, refusal visibility passed 16/16, tickets passed 30/30, and agent outcomes passed 26/26. Additional recorded tallies were 9/9, 10/10, 47/47, 4/4, and 34/34. Type checking was clean. UI, zombie-busy, running-snapshot, and reload-live-summary checks were named without recorded results.

## Implementation notes

The strip renders a server-authored snapshot of currently running sessions. Active main-session work receives a main row, aligning the agents strip with the product's other live-state surface while preserving the rule that a running indicator requires a live process.

## Verification plan

Use a real browser and a low-cost session to observe a genuinely long turn. Confirm that main work appears with honest elapsed time, settles at turn end, and also settles after a mid-turn kill. Confirm that subagent backfill and turn-end settling still behave correctly. The original plan also named UI, type checking, stale-card, and reattach checks.

## Risks

A reconstructed stale state must not create a false main row or fabricated duration. Turn completion and process death must both clear running state without disrupting subagent rows.
