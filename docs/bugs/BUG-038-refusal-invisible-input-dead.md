```orchard-ticket
{
  "id": "BUG-038",
  "type": "bug",
  "title": "Refused messages left the composer appearing unresponsive",
  "summary": "Refusals now explain what happened beside the composer while preserving the message for retry. The corrected behavior passed the refusal cases that previously exposed the invisible response, and the existing busy-session behavior remained intact.",
  "impact_if_we_wait": "Users may mistake an intentional refusal for a broken input and repeatedly retry without guidance. Bounded: this affects refusal feedback and display-correctness, not message data; the text remains in the composer.",
  ...
  "decision_history": [],
  "success_criteria": [
    "Every refusal path displays a plain-language explanation beside the composer",
    "Refused message text remains available for retry",
    "The composer remains enabled after a refusal",
    "Retrying after the server finishes succeeds",
    "No refusal or rollback branch reports only through the console"
  ],
  "code_refs": [],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/BUG-038-refusal-invisible-input-dead.md",
    "sha256": "aecb5188c0f3829146a5de6d41ddf97d89d5eed2ad437097c62836b6a011895a",
    "bytes": 13732,
    "original_title": "turn refusal is invisible: message stays in the box, nothing happens, only console says why",
    "migrated_on": "2026-08-19",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the archived ticket text; the symptom, affected refusal paths, visible-feedback requirements, retry behavior, bounds, and executed evidence remain present.",
    "dropped": []
  }
}
```

# BUG-038 — Refused messages left the composer appearing unresponsive

## Diagnosis

Server-side refusal preserved the message correctly but explained the refusal only in the browser console. The composer showed no bubble, queue entry, status line, or error, so an intentional refusal looked like an unresponsive input. The affected branches included `rollBackPendingStart`, frameless-bridge refusal, and drain refusal.

## Evidence

The matched correctness cases improved from 1/4 before the fix to 4/4 afterward. `verify:refusal-visible` passed 16/16, and `verify:zombie-busy` passed 34/34 across cases A–D without regression. Additional matched tallies were 4/4, 5/5, and 7/7. Typecheck was reported clean.

## Implementation notes

Each refusal path now presents a visible plain-language line explaining the reason, confirming that the text stayed in the composer, and telling the user when to retry. An attributed card matching BUG-031 was considered, but a status line was sufficient.

## Verification plan

Drive real drain and frameless refusals in a browser. Confirm the reason appears in the interface rather than only the console, the text remains, the composer stays enabled, and retry succeeds after draining. `verify:resume-refusal` and `verify:ui` were named as anti-regressions but have no recorded result.

## Risks

A refusal branch omitted from the shared visible-feedback handling could regress to console-only reporting while preserving server correctness.
