```orchard-ticket
{
  "id": "BUG-105",
  "type": "bug",
  "title": "Live background work was recorded as dead at turn end",
  "summary": "A worker dispatched in the background handed work to a helper, and when the dispatching turn ended the station wrote the helper down as ended for no known reason and removed it from the live list. The helper was still working. The sweep now leaves such work alone until its own ending arrives.",
  "impact_if_we_wait": "The orchestrator is briefed that a step it dispatched failed, so it distrusts finished work or dispatches it again. Bounded: this is a record-keeping fault in one turn-end sweep. The work itself still runs and still lands.",
  "current_need": "Restart the service on :4317 so the corrected sweep becomes live, and obtain the eighth independent clean-room reading before treating the record as settled.",
  "severity": "high",
  "area": "Agent liveness reporting",
  "reported": "2026-08-18",
  "reported_by": "agent",
  "owner": "agent",
  "work_state": "done",
  ...
  "decision_history": [],
  "success_criteria": [
    "A helper still working under a live background dispatcher is never written down as ended",
    "A helper whose whole ownership is finished has its ending recorded at the next boundary",
    "A helper started directly by the orchestrator is never spared, because it has no ancestry",
    "A genuine ending carried by the work's own final report is still recorded, exactly once",
    "An outside-imposed teardown never establishes or writes a per-item ending"
  ],
  "code_refs": [
    {
      "path": "src/server/agent-bridge.ts",
      "symbol": "case 'result' sweep",
      "note": "the per-row ownership guard was gated on `a.kind === 'tool'`, inherited from BUG-096's bash incident; ownership liveness is a fact about the chain, not the row kind"
    },
    {
      "path": "src/server/agent-bridge.ts",
      "symbol": "#chainHasLiveBackgroundAgent",
      "note": "positive-evidence-only predicate; the un-observed-owner degrade was deliberately not extended to agent rows"
    },
    {
      "path": "src/server/agent-bridge.ts",
      "symbol": "#backgroundTasks",
      "note": "became a derivation of raw level minus veto so frame order cannot change the answer"
    },
    {
      "path": "src/server/open-tool-calls.ts",
      "symbol": "ownerChainOf",
      "note": "read only; unchanged by this ticket"
    }
  ],
  "related": [
    {
  ...
  "verification": [
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a015a7-c2a0-7603-82bc-2d08f9af0f15",
      "verdict": "broken",
    ...
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a0163c-ad70-7fc1-972f-c787815d4085",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a01679-55fd-7070-bf2a-ba34c56f6750",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a016a8-d945-7a12-bd9b-210573f60769",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a016ce-844f-7a11-b2c9-00a617973683",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "anthropic",
      "model": null,
      "run_id": "28c7b7da-927e-4c29-9850-9b62d84d8f5a",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a01738-b7a7-75a3-9278-8d153c0df8ca",
      "verdict": "broken",
      "verdict_on": "2026-08-18",
      "harness": "scripts/independent-verify.mjs"
    }
  ...
  "source": {
    "archived_path": "docs/bugs/archive/BUG-105-foreground-subagent-of-a-live-owner-is-recorded-dead.md",
    "sha256": "8c19bfec38566d40e63906bb298e57dd7c7ed6caada0f64cfe1abe1fcdf9fcab",
    "bytes": 94968,
    "original_title": "an agent that is still working is recorded as dead, because the agent that dispatched it works in the background",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared clause by clause against the original head: the symptom, the kind-gate diagnosis, the four asserted properties, the BUG-096 contract deltas, the seven verdict rounds and the restart dependency are all present.",
    "dropped": [
      "the running header stack's repeated 'still awaiting an Nth verify' phrasing, which the verification records now carry",
      "the reported-by lane's account of finding this while answering a different question"
    ]
  }
}
```

# BUG-105 — Live background work was recorded as dead at turn end

## Diagnosis

### The mechanism

`task_started` classifies a lane as `kind:'agent'` when it carries a `subagent_type` (or `task_type === 'local_agent'`), else `kind:'tool'`. The turn-end sweep skips any row the engine's background level lists (`#backgroundTasks`) or that was born from a `run_in_background` dispatch (`#bgBornTasks`). Everything else still `running` is settled and passed to `outcomes.turnEndOutcome`, which with no completion evidence returns the honest `unknown` death.

A foreground subagent of a background worker is on neither list: it is not background work, it is a child of work that is. ARCH-003's per-row ownership guard resolves the lane's whole ownership chain and asks whether any ancestor is a live background agent, but it was gated on the row's kind:

```ts
const ownedByLiveBackground = a.kind === 'tool' && this.#ownershipSpares(ownerChain);
```

That `a.kind === 'tool'` was inherited from BUG-096, whose incident happened to be a child bash. Nothing about ownership liveness is kind-specific.

### Why this is not ARCH-003 again

ARCH-003 is about which agent owns a tool call, and that machinery already answers correctly. This is the removal of a restriction on who may benefit from that answer. It is also the likelier shape: ARCH-003's remaining case needs a background worker to dispatch a *background* sub-worker, against house style; this one needs a background worker to dispatch a subagent in the foreground, which is exactly what house style asks for.

### Why a `continue` rather than settle-and-hold-the-write

For a `local_bash` row settling is right — BUG-030 found those spinning forever and the engine closes them with their own notification. For an agent row, settling is itself a report that live work ended: the row leaves the live strip, and a settled lane is terminal evidence to `reap()`, so the subagent's ownership record is dropped and the next bash it issues resolves on a truncated chain and has its death fabricated too.

### What collides with the BUG-096 contract

Only the row-kind clause changes. A background bash owner still never licenses suppression. A retired background agent still never licenses suppression. The settle-but-hold-the-write clause stays as it is for tool rows and is not extended to agent rows.

### Unrepresentable versus merely guarded

Unrepresentable: a main-thread lane spared by any arrangement of live agents (it has no ancestry, and both rules are `.some()` over ancestry); an agent row spared on absence of knowledge (the predicate has no `false ⇒ spare` branch); a spared row's death being lost rather than deferred, given that any further boundary occurs.

Guarded, not eliminated: the ownership answer under `tool_use` id reuse — a main-thread re-issue always wins over a stale subagent record.

Stated, not fixed: if a session never takes another turn, a spared row stays `running` and no ending is recorded. That is the pre-existing posture for every background lane since BUG-037's fix, and it is the stall detector's territory — BUG-046 and BUG-033 — not the sweep's.

## Evidence

Probed live on the real bridge while the ARCH-003 lane was working: `allEnded=["fgSub:unknown", …]`, where `fgSub` was a running foreground subagent of a background worker the engine still listed as live.

Seven independent clean-room readings each returned BROKEN and each named a distinct hole, every one reproduced live before being fixed:

1. Stale background-level membership spared a retired owner's child forever.
2. `b0dfaf6` regressed: a task reporting `task_updated completed` before its row existed was recorded as an `unknown` death, because the status was dropped at `if (!a) return`. Fixed at **28/28 exit 0**, must-FAIL 27/28 exit 1 reproduced first.
3. Two disagreeing terminal reports arriving before the row existed collapsed to the first, leaving no ledger row. Fixed at **40/40 exit 0**, must-FAIL 35/40 exit 1 on unmodified `0fc87c7`.
4. A teardown `task_notification status:"stopped"` arriving as the first frame for a task was promoted into a `killed` death. Fixed at **56/56 exit 0**, six new checks failing on an unmodified worktree at `cf6c97e`.
5. A row-less teardown blanket wrote a permanent liveness veto, starving a task the engine later re-announced as live (`reann:unknown`, `reannChild:unknown`); detached, the starved level made `workLifetime()` answer no. Fixed at **73/73 exit 0**, eight new checks failing live on unmodified `991ac79`.
6. A row-less `task_notification status:"completed"` — the only terminal frame a tool task ever gets — established no outcome, so a later `task_started` resurrected a task the engine had reported finished (`snap2.running = ['bashDone','bashChild']`). Fixed at **84/84 exit 0**.
7. The demotion from reading 6 still converted a teardown into a per-task `killed` where the row was already present (`ended=["killed"], rows=0`, measured live through the real server and bridge). Fixed at **95/95 exit 0**, with nine checks must-FAIL on `08ecda4`, two of them in a real headless browser.

The original repro, `verify-bug-105-foreground-subagent-of-live-owner`, ran 6/9 exit 1 on unmodified `cb2ceb2` with the fabricated `fgSub:unknown` printed, and 9/9 after the fix; six of nine checks pass on the unfixed code, so the suite isolates the missing half rather than restating a strawman. Alongside it: BUG-096's contract suite at 17/17, its background-child lane at 17/17, all four ARCH-003 guard suites, the liveness conformance suite at 96/96, the running-snapshot suite at 47/47, the stall detector at 33/33, stale agent cards at 12/12, close-background-detach at 27/27, agent outcomes at 45/45, and the BUG-037, BUG-044, drain-truth, outcomes-rail, detach and reattach-backfill suites. Typecheck stayed clean throughout.

## Implementation notes

The final shape is one rule with no row-state arm: a teardown blanket never establishes or writes a per-task outcome, row or no row. What happens at a genuine teardown falls out of the ordinary inference path, which records honestly and exactly once. The line is drawn by what a frame *says*, not by its type — a self-reached ending (`completed`/`failed`) is the engine's per-task verdict and settles; an outside-imposed one (`stopped`/`killed`) stays a withdrawable blanket. An established per-task outcome is never withdrawn; a blanket-only veto is withdrawn by the task's own `task_started`. A row-less notification is refused once `#establishedOutcomes` holds a verdict, keeping both frame orderings' ledgers identical.

Agent rows are spared only on positive evidence that an ancestor is a live background agent, via `#chainHasLiveBackgroundAgent`. The un-observed-owner degrade `#ownerIsUntrackedSubagent` is deliberately not extended to them: its justification is kind-specific, resting on a real-CLI capture that a subagent-issued bash only ever gets a task row from a background agent, and no equivalent fact licenses silencing a whole class of agent endings on absence of knowledge.

## Verification plan

Each clean-room reading's own script is reproduced as a must-FAIL against the unmodified tree first, then re-run after the fix. The spare is level-triggered and re-asked at every boundary, so the deferred-never-lost property is asserted by dropping the owner off the level and checking the honest ending appears at the next sweep. An eighth independent clean-room reading is still wanted, and the bridge is in-process, so the change reaches users only after a `:4317` restart.

## Migration and rollback

Deploy requires a `:4317` restart; the bridge runs in-process and a running server keeps the old sweep.

## Risks

The ownership answer under `tool_use` id reuse is guarded rather than eliminated, and is now asserted for the agent row class too. A session that never takes another turn leaves a spared row `running` with no ending ever recorded — the pre-existing posture for every background lane, tracked as stall-detector work rather than sweep work.
