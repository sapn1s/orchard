```orchard-ticket
{
  "id": "BUG-119",
  "type": "bug",
  "title": "Finished tickets migrated to a record denying any proof existed",
  "summary": "Tickets whose own status line said they were verified came out of the conversion saying nothing was recorded, and the reader-facing sentence repeated the denial. The status line now decides that state, and each ticket's own executed evidence is handed to the writer, so the migrated text says what ran. A separate change later removed the state field entirely.",
  "impact_if_we_wait": "Readers of a converted board cannot tell proven work from unproven, and the record reads as evasion. Bounded: this is a transcription fault in the converted copy only. The original tickets, their logs and the independent verdicts are untouched.",
  "current_need": "Nothing is outstanding here. The migration suite and a new evidence suite both passed, a case that failed before the change passed after it, and typecheck stayed clean.",
  "severity": "high",
  ...
  "decision_history": [],
  "success_criteria": [
    "A ticket whose status line claims verification does not migrate to a not-recorded state",
    "A finished ticket's migrated need sentence names the proof that closed it",
    "Independent verdicts stay a separate signal from the fixer's own suite runs",
    "Tickets with genuinely no evidence still migrate to the not-recorded record"
  ],
  "code_refs": [
    {
      "path": "scripts/migrate-tickets.mjs",
      "symbol": "extractGivens",
      "note": "read verification state from `Verified-by:` lines only, discarding the status-line classification it already computed for work_state"
    },
    {
      "path": "scripts/migrate-tickets.mjs",
      "symbol": "buildPrompt",
      "note": "hard-ordered the placeholder: 'verification_state must be \"not_recorded\" or \"not_required\"'; also emitted the oversized verbatim-token block whose per-token labels leaked as seams"
    },
    {
      "path": "scripts/lib/ticket-schema.mjs",
      "symbol": "classifyLegacyStatus",
      "note": "already maps VERIFIED to holds, FIXED to pending, NOT-A-BUG to not_required"
    },
    {
      "path": "scripts/lib/ticket-schema.mjs",
      "symbol": "validateTicket",
      "note": "rejected `holds` whenever the record list was empty, contradicting the file's own header docstring"
    },
    {
      "path": "scripts/verify-bug-119-verification-evidence.mjs",
      "symbol": null,
      "note": "new suite covering the self-evidence extraction"
    },
    {
      "path": "scripts/lib/ticket-writing.mjs",
      "symbol": "WORKED_EXAMPLES",
      "note": "the second worked example's need sentence modelled the denial being copied"
    }
  ],
  "related": [
    {
  ...
  "verification": [
    {
      "provider": "openai",
      "model": null,
      "run_id": "01a01bb8-e94a-7db3-9cf4-9af31079fa6e",
      "verdict": "broken",
      "verdict_on": null,
  ...
  "source": {
    "archived_path": "docs/bugs/archive/BUG-119-migration-writes-not-recorded-over-real-verification-evidence.md",
    "sha256": "374201a1cfcaeada3a1d4ce97b45956c4141b8023eb2884907474b87073c12b2",
    "bytes": 38050,
    "original_title": "Migrated tickets say verification is \"not recorded\" on tickets whose own status says VERIFIED",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared field by field against the original head: all three defects, the survey table, the rejected widening, the built approach and the blocker are present.",
    "dropped": [
      "the dispatch-class note restating that cause and blast radius are both nameable",
      "the inline JSON snippet of the migrated BUG-085 record, whose content is stated in prose"
    ]
  }
}
```

# BUG-119 — Finished tickets migrated to a record denying any proof existed

## Diagnosis

### Three defects, one visible symptom

**The state came from the wrong signal.** `extractGivens` derived `verification_state` from `verificationStateFrom(records)`, and those records are `Verified-by:` dispatch lines only — 16 files on the whole board carry one. `classifyLegacyStatus` already returns a verification state per status token (`VERIFIED` → `holds`, `FIXED` → `pending`, `NOT-A-BUG` → `not_required`), the pipeline already calls it for `work_state`, and it threw the other half away.

**`validateTicket` then made the right answer unrepresentable.** Its rule — an empty record list forces `not_recorded` or `not_required` — rejected `holds` for the 121 tickets that say VERIFIED without an independent verdict. The prompt's hard order was downstream of that rule, not the cause. `ticket-schema.mjs`'s own header docstring states the intended mapping, so the validator contradicted the file it lives in.

**The verbatim-token block was about four times larger than provenance needs.** Its per-token labels came back out in the prose as seams — "on date 2026-08-14", "ticket ARCH-006 was reported on date 2026-08-18". 653 of the 887 head-provenance tokens already sit inside the activity log, which is copied byte-for-byte and asserted by `provenanceCheck`, so re-placing them bought nothing and cost a seam each.

### The approach, and the one rejected

The corpus expresses two different kinds of evidence and the board's method rests on telling them apart: an independent clean-room verdict versus the fixer's own executed suite. Widening `extractVerificationRecords` to emit `verification[]` entries for suite runs would have satisfied the literal ask, but `verification[]` is typed `{provider, run_id, verdict}` — a suite run has neither a provider nor a run id, so both would be invented, and an empty `verification[]` would stop meaning "nobody independently verified this".

Built instead: the status line decides the state, and the extracted self-evidence goes to the model as givens so it writes what the proof was. `verification[]` keeps its exact meaning and stays empty on 169 tickets, which is true.

### What changed underneath afterwards

[[ARCH-009]] was decided and built, and `verification_state` no longer exists as a field. The field half of this bug dissolved rather than being fixed — there is nothing left to write the placeholder into. The prose half is what this ticket owns: a finished ticket's own evidence is extracted from the whole file, log included, and put in front of the model.

## Evidence

### The symptom

A migrated `BUG-085` carried `work_state: verified` beside `verification_state: not_recorded`, with the need sentence reading "No further action is currently recorded; the fix is marked verified." Its status line said VERIFIED and its activity log recorded executed suites with pass tallies. The user read this as the pipeline evading rather than reporting. A writing contract tested against the same output by another lane made no measurable difference — a 2–2 blind split — because the placeholder was ordered by the prompt, not chosen by the writer.

### Repro

`node scripts/migrate-tickets.mjs --ids BUG-085`, then read the staged record and the `buildPrompt` order that produces it.

### Survey of 185 tickets, ARCH-005 excluded

| Signal | tickets | of the 152 finished-with-no-record |
|---|---|---|
| `Verified-by:` dispatch line (what the extractor read) | 16 | 0 |
...
Only 8 of the 152 have no verification evidence of any kind — `ARCH-002`, `DEPLOY-003`, `FEAT-005`, `FEAT-017`, `FEAT-026`, `FEAT-028`, `FEAT-036`, `FEAT-046` — every one a superseded, won't-do, audit-only or decision-only closure, where `not_recorded` is the correct record.

### What was run on the fix

`verify:bug-119` passed 102/102 and the new `verify:bug-119-verification-evidence` passed 48/48. `verify:migrate-tickets` passed 51/51, `verify:ticket-schema` 53/53, `verify:board-tool` 34/34, and `verify:ticket-writing-contract` and `verify:ticket-view-redesign` 184/184 each. A pre-fix FAIL was recorded before the change, and typecheck was clean. Harness runs `01a01bb8`, `01a01bc8` and `01a01bda` cover this work.

## Implementation notes

The status line, which the board already trusts to decide `work_state`, was made to decide the verification half too, using the classification `classifyLegacyStatus` was already returning and the pipeline was already discarding. `validateTicket`'s empty-records rule was relaxed to match the docstring in the same file. The self-evidence extractor reads the entire ticket file including the activity log, so executed suites, pass tallies, pre-fix failures and clean standing checks reach the writer as givens. The verbatim-token block was cut to what provenance actually requires and its per-token labels removed, since the activity log is copied byte-for-byte and asserted by `provenanceCheck`.

`public/app.js` and the ticket view were held by another lane, so no new rendered schema key could be added here.

## Verification plan

Run `npm run verify:migrate-tickets` and `scripts/verify-bug-119-verification-evidence.mjs`. Assert that a VERIFIED status line never migrates to a not-recorded state, that the eight evidence-free closures still do, and that a finished ticket's migrated need sentence names its own proof rather than denying one exists.

## Risks

The independent clean-room dispatch on this work returned BROKEN, and independent verification of the [[ARCH-009]] build behind it is still outstanding. A model-backed migration run across the corpus remains the highest-value untaken check — the suites cover the extraction, not the writing that consumes it.
