```orchard-ticket
{
  "id": "FEAT-015",
  "type": "feature",
  "title": "Sessions died whenever the server was restarted",
  "summary": "Restarting the server used to end every session running at the time, interrupting the turn in flight. Sessions started directly on this machine now survive a restart and reconnect afterwards. A session's conversation continues from disk after the restart rather than picking up the turn that was interrupted.",
  "impact_if_we_wait": "Every restart cost people their in-flight turns during active development. Bounded: conversation history always lived on disk and was never lost, so the harm was the current turn and the interruption, not the thread.",
  "current_need": "Nothing is outstanding. The detach, restart-survival, reattach-approval and interface behaviours were all exercised and passed, with standing checks clean.",
  "severity": "high",
  "area": "Session survival across restarts",
  "reported": "2026-08-03",
  "reported_by": "user",
  "owner": "agent",
  "work_state": "verified",
  ...
  "decision_history": [
    {
      "asked_on": "2026-08-03",
      "question": "Should we live with restarting only between turns, or decouple sessions from the server?",
      "mode": "single",
      "options_keys": [
        "A",
        "B"
      ...
      "chosen_by": "agent",
      "note": "Short-term guidance was to restart at pauses and lean on the idle watcher; the durable decoupling was chosen and built, leaving one transport gap."
    }
  ],
  "success_criteria": [
    "A session running at restart time is still running afterwards",
    "The server reconnects to sessions it finds still running after boot",
    "A restart loses at most the turn in flight, never the conversation"
  ],
  "code_refs": [
    {
      "path": "src/server/processes.ts",
      "symbol": null,
      "note": "already scans running processes; the model for rediscovering sessions after boot"
    },
    {
      "path": "src/server/bridge.ts",
      "symbol": "spawnClaudeCodeProcess",
      "note": "the hook used to spawn detached and to reconnect a running session"
    }
  ],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/FEAT-015-server-restart-survives-live-sessions.md",
    "sha256": "e27d3827d51207e083ef6efd8bf37e195f5042f97ede52272674bdaa0423c0ea",
    "bytes": 17753,
    "original_title": "Restart the server without killing live sessions",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the original head: the question, the process-ownership diagnosis, the rarity assessment, the three-step decoupling design, the chosen path and the residual transport gap are all present.",
    "dropped": [
      "the idle-watcher restart mitigation, which the shipped decoupling makes moot",
      "the phrasing of the original short-term versus long-term recommendation, now recorded as answered history"
    ]
  }
}
```

# FEAT-015 — Sessions died whenever the server was restarted

## Diagnosis

### Why a restart killed sessions

The live bridges and the command-line processes they spawned lived inside the server process. Killing the server therefore killed the whole tree. The earlier detach and reattach work only covered a dropped socket — a browser reload or a tab switch — which is a different failure from the server process itself going away.

Conversation history was always on disk, so a resume after a restart lost the current turn and nothing else.

## Evidence

### What ran

Four suites were executed against the implemented change: detach 7/7, restart-survives 15/15, reattach-approval 11/11, and the interface suite 3/3. Typecheck stayed clean. A further close-busy-detach suite is named in the ticket but has no recorded run.

## Implementation notes

### The shape that was built

Each session's process is spawned into its own process group so it outlives the server. On boot the server rediscovers the still-running processes and reconnects to them, so the bridge is a reconnectable proxy rather than the owner of the subprocess.

One transport gap is recorded honestly: a follow-up turn cannot be delivered live to the still-running process after a restart. The thread continues by resuming from disk instead.

## Verification plan

### The bar

Start a session, restart the server mid-turn, and confirm the session process is still alive and reattaches. Confirm the conversation continues afterwards with only the interrupted turn missing.

## Risks

### Residual

The surviving process cannot receive a live follow-up turn over the restored transport; continuation goes through a resume from disk. Sessions other than the directly-driven kind were not covered by this work.
