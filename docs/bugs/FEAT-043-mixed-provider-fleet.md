```orchard-ticket
{
  "id": "FEAT-043",
  "type": "feature",
  "title": "Sessions can now run agents on either provider by strength",
  "summary": "A session can dispatch work to either model provider and pick per task, so both subscriptions are used at once. A shared routing guide records which provider suits which kind of work, and a dated staleness check flags it when it ages. Choosing the provider per dispatch from the Agent screen was not built.",
  "impact_if_we_wait": "The mixed fleet works from the command line, so nobody is blocked. Bounded: only the in-app per-dispatch picker is missing, and one provider path runs without live progress and with a weaker sandbox. No routing guidance or existing session behaviour is at risk.",
  "current_need": "Nothing is outstanding. The dispatch runner, the routing guide and its staleness check all shipped, each with its own passing suite and a live cross-provider review.",
  "severity": "medium",
  "area": "Multi-provider agent dispatch",
  "reported": "2026-08-05",
  "reported_by": "user",
  "owner": "you",
  "work_state": "done",
  ...
  "decision_history": [],
  "success_criteria": [
    "A session's orchestrator can run on either provider",
    "Each dispatched agent can be sent to either provider per task",
    "A dated routing guide records per-capability provider recommendations",
    "The consolidation pass flags the routing guide when its dated header goes stale",
    "Routing guidance reaches launched sessions in condensed form"
  ],
  "code_refs": [
    {
      "path": "scripts/dispatch.mjs",
      "symbol": null,
      "note": "the cross-provider dispatch runner"
    },
    {
      "path": "scripts/verify-dispatch.mjs",
      "symbol": null,
      "note": "fixture suite for the dispatch runner"
    },
    {
      "path": "scripts/wa-consolidate.mjs",
      "symbol": null,
      "note": "checks the routing guide's staleness header on every pass and via --check-routing"
    },
    {
      "path": "ROUTING.md",
      "symbol": null,
      "note": "canonical in the methodology repo, mirrored here"
    }
  ],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/FEAT-043-mixed-provider-fleet.md",
    "sha256": "7b7f575bf42903624087b9d585409ff3c38e79fa3758de5ce548179d84a7c5a4",
    "bytes": 35191,
    "original_title": "mixed-provider fleet: orchestrator AND dispatched agents can each be GPT or Claude, routed by strength",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared field by field against the ticket head: the three goals, the phase breakdown, both constraints, the shipped evidence and the two named boundaries are all present.",
    "dropped": [
      "the verbatim quote from the original report",
      "the enumerated list of research capability axes and named benchmarks, kept in condensed form"
    ]
  }
}
```

# FEAT-043 — Sessions can now run agents on either provider by strength

## Diagnosis

Both providers were connected but used as alternates. Nothing let one session's orchestrator sit on one provider while dispatching agents to the other, and the per-capability model advice existed only as undated folklore.

The unit of the mixed fleet is the Orchard-dispatched agent, not a provider's own internal subagents: Codex reports `subagents:false`, so the design deliberately sits at the session/Agent-tool level.

## Evidence

`verify:methodology-sync` 3/3 and `verify:wa-selfmaintain` 39/39 passing, with typecheck clean. Recorded tallies of 18/18 and 5/5 accompany the routing-injection and related work. `verify:dispatch`, `verify:routing-inject`, `verify:codex-runtime`, `verify:orchard-transcripts` and `verify:ui` are named in the ticket without a result attached here. The Status line reports the dispatch fixture at 15/0, then 24/0 after hardening, plus a live cross-provider review.

## Implementation notes

Phase R produced a dated comparative routing table, sourced from official evals and credible independent benchmarks, with contested claims marked. It became canonical `ROUTING.md` in the methodology repo, with a synced mirror here and a condensed form injected into launched sessions.

The v1-hardening pass fixed the anthropic-path limitations the live review accepted: timeout, sandbox and taxonomy. `npm run dispatch`, `verify:dispatch` and `verify:routing-inject` are applied in package.json.

Routing is meant to weigh budget pressure as well as skill, since Anthropic quotas and rolling ChatGPT windows exhaust differently.

## Verification plan

Re-run the dispatch fixture suite and the routing-injection suite. Confirm the consolidation pass still flags a back-dated staleness header, both on the boot-time pass and standalone.

## Risks

Two documented boundaries remain on the anthropic dispatch path: the sandbox is application-level rather than OS-level, and the path is non-streaming, returning a single JSON result with coarser progress than the openai path. The routing table's value decays each model generation; the staleness check is what stops that going unnoticed.
