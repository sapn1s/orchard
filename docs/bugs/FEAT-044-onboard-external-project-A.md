```orchard-ticket
{
  "id": "FEAT-044",
  "type": "feature",
  "title": "First outside project brought onto the shared orchestrator stack",
  "summary": "A project outside the main repository now runs on the shared orchestrator setup end to end. One onboarding action scaffolded its ticket board and instruction file, the dashboard registered it, and a real low-cost session booted with the shared working agreement, a board snapshot and routing guidance. That session quoted the injected board focus line back in a live turn.",
  "impact_if_we_wait": "Nothing further is at risk; the chain already runs live. Bounded: this was a one-off adoption exercise, it touched only opt-in scaffolding files, and the other project's own application code was never modified.",
  "current_need": "Nothing is outstanding. Onboarding, fleet sync and scope checks all ran green, and the launched session proved boot awareness by quoting its board.",
  "severity": "medium",
  ...
  "decision_history": [],
  "success_criteria": [
    "Onboarding artifacts exist on disk in the external project",
    "The board consistency check passes inside the external project",
    "The external project appears in the dashboard",
    "A launched session receives the shared agreement, board snapshot and routing section",
    "A live turn references the injected board content"
  ],
  "code_refs": [
    {
      "path": "scripts/onboard.mjs",
      "symbol": null,
      "note": "scaffolds the board, instruction file and guard in a target project"
    }
  ],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/FEAT-044-onboard-external-project-A.md",
    "sha256": "93a786d292ff1b13ea16a907ddbd0df85c378d9ea3d435d83ff7b5a4c89c53ed",
    "bytes": 16338,
    "original_title": "onboard external-project-A: first external project on the full Orchard stack",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared against the original head: the goal, the do-not-modify and no-restart constraints, all four verification requirements, and the second-consumer observation task are present above.",
    "dropped": [
      "the original phrasing of the 2026-08-04 ask, kept only as the reported date",
      "the parenthetical list of contributing ticket ids, now in related[]"
    ]
  }
}
```

# FEAT-044 — First outside project brought onto the shared orchestrator stack

## Diagnosis

The onboarding scaffolder, the shared working agreement, the local conventions layer from FEAT-039 and the routing/dispatch layer from FEAT-043 had each been proven only inside their home repository. Whether they compose for an unrelated checkout was untested until this ticket exercised the whole chain against a real external project.

## Evidence

The onboard suite passed 36/36, fleet sync 21/21, scope checking 12/12, with a further 8/8 tally recorded, and typecheck stayed clean. Beyond the suites, the board consistency check ran green inside the external project itself, the project appeared in the dashboard, and a real cheap-model session was launched and quoted the injected board focus line in its first turn.

## Implementation notes

Only opt-in files were written into the external checkout: the ticket board directory, the instruction file produced by the scaffolder, and an optional conventions stub. Application code there was untouched. Registration went through the normal add-project call on the running dashboard at :4317, so nothing had to be restarted. Any commit in the external repository was gated on a clean working tree containing exclusively onboarding files.

## Verification plan

Confirm the artifacts on disk and a green board check inside the external project, confirm dashboard visibility, then launch a real low-cost session and assert from the composed instruction surface that the agreement, board snapshot and routing section are present. One small live turn referencing the board demonstrates boot awareness.

## Migration and rollback

The scaffolding is additive and confined to opt-in paths, so reverting means deleting the added files and removing the dashboard registration.

## Risks

This was also the declared trigger for the deferred multi-consumer methodology story: a second consumer of the shared agreement reveals how the sync gap behaves in practice. Observations were gathered rather than a sync mechanism being built speculatively, so that gap remains as it was.
