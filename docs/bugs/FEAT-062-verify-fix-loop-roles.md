```orchard-ticket
{
  "id": "FEAT-062",
  "type": "feature",
  "title": "Repeated verification rounds threw away the fixer's context",
  "summary": "Each round of checking a fix used to start the fixer from nothing, so everything it had already learned was discarded and the same dead ends were re-explored. The fixer thread now persists across rounds while each checker is created fresh, and a staleness gate refuses to resume a fixer whose files moved under it.",
  "impact_if_we_wait": "Repair rounds cost more and repeat work already done, and a resumed fixer can act on a repo that changed beneath it. Bounded: this affects the repair loop's efficiency and correctness of resumption, not stored tickets or shipped code.",
  "current_need": "Nothing is outstanding. The loop crashed before the fix when relaying an oversized result, and the corrected path delivered it in full, with the clean-room pass raising its own novel deletion case that the staleness gate caught.",
  "severity": "medium",
  "area": "Verify and repair loop",
  "reported": "2026-08-10",
  "reported_by": "user",
  "owner": "you",
  "work_state": "verified",
  ...
  "decision_history": [],
  "success_criteria": [
    "With the repo unchanged, the same fixer thread resumes and repairs the reported finding",
    "A material change to a file the fixer touched forces re-orientation or a fresh fixer, never a silent resume",
    "The round cap escalates instead of looping further",
    "Every checking round uses a new dispatch, with no reuse asserted",
    "Findings reach the fixer word for word, with no paraphrase in between"
  ],
  "code_refs": [
    {
      "path": "scripts/lib/fix-loop.mjs",
      "symbol": "fixRound",
      "note": "routes an oversized composed verdict over the standard input path instead of an argument list"
    },
    {
      "path": "scripts/dispatch.mjs",
      "symbol": null,
      "note": "the prompt-over-stdin entry both the checking hop and the fixer hop now use"
    }
  ],
  "related": [
    {
  ...
  "verification": [
    {
      "provider": "anthropic",
      "model": "sonnet",
      "run_id": "f5f8de18-32d8-45cc-8996-1135a85c3b74",
      "verdict": "broken",
    ...
    },
    {
      "provider": "anthropic",
      "model": "sonnet",
      "run_id": "9e477a62-be6d-4695-8981-88d95466d2fc",
      "verdict": "broken",
      "verdict_on": "2026-08-11",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "anthropic",
      "model": "sonnet",
      "run_id": "87b38f59-732f-4f1b-9aec-56b6be262624",
      "verdict": "broken",
    ...
    },
    {
      "provider": "anthropic",
      "model": "sonnet",
      "run_id": "175cf18f-f72c-41ae-9614-fa638fa07775",
      "verdict": "broken",
      "verdict_on": "2026-08-12",
      "harness": "scripts/independent-verify.mjs"
    },
    {
      "provider": "anthropic",
      "model": "sonnet",
      "run_id": "78fb3b76-6bc3-41f0-81b0-737328e704cd",
      "verdict": "holds",
      "verdict_on": "2026-08-12",
      "harness": "scripts/independent-verify.mjs"
    },
  ...
  "source": {
    "archived_path": "docs/bugs/archive/FEAT-062-verify-fix-loop-roles.md",
    "sha256": "e019f48e7dc79b65b702b7346e013a663239dba238e067273f4f1033e523ae42",
    "bytes": 86749,
    "original_title": "verify→fix loop: persistent fixer thread, ephemeral clean-room verifier, staleness gate",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Compared field by field against the original head: the two lifetimes, the staleness thresholds, the round cap, the three named risks, the five-part fixture and the relay repair are all present.",
    "dropped": [
      "the closing-bar wording about attempt counts and which verdict decides, which the pipeline's own verdict records carry",
      "the verbatim quotation of the original report, whose substance is the staleness objection now recorded in the diagnosis"
    ]
  }
}
```

# FEAT-062 — Repeated verification rounds threw away the fixer's context

## Diagnosis

### Two lifetimes on purpose

The fixer is a durable thread, resumed across rounds so its accumulated knowledge of code paths and already-eliminated theories survives. The checker is ephemeral and clean-room every round: a fresh dispatch, no project injection, no memory of prior rounds, no exposure to the fixer's rebuttals. Persisting the checker would let it converge with the fixer, which is exactly what the separation exists to prevent.

### The staleness gate

A resumed fixer's picture of the repo can be stale — a sibling lane touches a shared file, or a stash-pop reverts another agent's work. At dispatch time the loop records the base commit, the files touched, and optionally hashes of the interfaces depended on. On relay-back it computes the delta: unchanged or trivially changed means resume with the findings verbatim; materially changed means either a mandatory re-orientation preamble naming exactly what moved, or a fresh fixer when the change is severe enough that the edited function no longer exists. The set of touched files is derived from the loop's own before/after snapshot tree-diff as of 2026-08-12, not from a status line delta.

### Round cap and escalation

A fixer is invested in its own approach, so repeated rounds risk argument rather than repair. The cap starts at two repair attempts after the first failure; on exhaustion the loop escalates to a fresh fixer, reclassifies the work, or surfaces to the user. Every round is recorded on the ticket so the loop is auditable.

### The relay defect

One clean-room round found that an oversized composed verdict crashed the fixer hop while being handed over as an argument list — the requirement was no such failure at any hop. The repair routes that hand-over through standard input, the same way the checking hop already did.

## Evidence

The must-FAIL case was proven before the change: the loop crashed on the oversized relay with nothing sent. After the change the same relay went over standard input carrying 170451 bytes, and the tail sentinel arrived intact.

The loop suite passes 35/35, up from 34 with the new fixer-hop ratchet. The independent-verification suite passes 132/132 and the dispatch suite 24/24. The gitless clean-room run passes 29/29 with the git-dependent case skipped. Typecheck and the leak gate are clean.

The closing clean-room attempt needed no re-prompt. Its own novel adversarial case — deleting a file the fixer had touched, uncommitted, midway through checking — fired the staleness gate as designed.

The code change is commit 66e092c.

## Implementation notes

The ticket carries the fixer's thread id and the checking run id, plus one log line per round recording the round number, the verdict, a pointer to the evidence, and the staleness decision taken.

## Verification plan

The fixture is a defect, a fixer, and a planted checker failure. With the repo unchanged, the loop resumes the same fixer thread and it repairs the finding. With a sibling commit materially changing a touched file, the loop re-orients with the change named or spawns fresh, and never silently resumes. The round cap triggers escalation rather than looping. Every checking round is a new dispatch id, with reuse asserted against. Findings are relayed verbatim, asserted by matching the checker's exact text inside the fixer's input. The whole fixture must fail before the change.

## Risks

Sunk-cost repair: a fixer may patch the specific case the checker built rather than the underlying defect. The countermeasure is that each round's checker is fresh and constructs its own adversarial case, so patching one case does not survive the next round.

Context rot: a long fixer thread accumulates its own wrong beliefs, which the round cap bounds.

False staleness alarms: a formatting-only change in a touched file should not trip the gate. The delta check has to stay semantic enough to avoid crying wolf, or people will learn to ignore it.
