```orchard-ticket
{
  "id": "FEAT-085",
  "type": "feature",
  "title": "Overlong replies persisted despite standing instructions",
  "summary": "Replies kept running long and drifting from the agreed format, and reminders in the prompt faded over a session. A check now runs at the end of each turn: a compliant reply closes normally, and a non-compliant one gets a corrective note naming what was missing. It only warns by default, and blocking is opt-in.",
  "impact_if_we_wait": "Format drift returns and a person has to re-flag it by hand each session. Bounded: this affects reply presentation only, not user data or session stability, and the check allows the turn to close whenever it errors.",
  "current_need": "Nothing is outstanding. Compliant replies passed untouched, non-compliant ones produced the correction, the loop cap and fail-open cases held, and standing checks stayed clean.",
  "severity": "medium",
  "area": "Response format enforcement",
  "reported": "2026-08-14",
  "reported_by": "user",
  "owner": "agent",
  "work_state": "verified",
  ...
  "decision_history": [
    {
      "asked_on": "2026-08-15",
      "question": "Should the end-of-turn check block a non-compliant reply, or only warn?",
      "mode": "single",
      "options_keys": [
        "A",
        "B"
      ...
      "chosen_by": "user",
      "note": "Advisory-only became the default because the grading of a reply has a known gap; blocking is available behind an opt-in environment flag."
    }
  ],
  "success_criteria": [
    "A compliant reply is allowed to close with no extra model round-trip",
    "A reply missing the digest or containing an emoji is named in a correction",
    "At most one correction per turn, never an indefinite loop",
    "A validator error or garbage input allows the turn to close",
    "The disable flag makes the check inert",
    "Normal sessions still end normally"
  ],
  "code_refs": [
    {
      "path": ".claude/settings.json",
      "symbol": "Stop",
      "note": "project-level hook registration, deliberately not the user's global settings so it is trivially removable"
    },
    {
      "path": "scripts/",
      "symbol": null,
      "note": "validator script; reuses the FEAT-083 digest parser rules (leading fence only, non-empty text)"
    }
  ],
  "related": [
    {
  ...
  "source": {
    "archived_path": "docs/bugs/archive/FEAT-085-stop-hook-enforces-response-format.md",
    "sha256": "fec81c46a2fe0326af35b94b9af9fded22877440cfd04416447954bc4adeb157",
    "bytes": 37154,
    "original_title": "Stop hook enforces the response format deterministically (check-first, re-prompt only on failure)",
    "migrated_on": "2026-08-20",
    "migrated_by": "migrate-tickets.mjs",
    "confirmation": "Checked field by field against the original head: the design, the deterministic checks, the loop cap, fail-open, the escape hatch, the proof bar and the FEAT-084 pairing are all present.",
    "dropped": [
      "a passing note that a deterministic cap on concurrent subagent fan-out is worth its own ticket, which the original itself defers"
    ]
  }
}
```

# FEAT-085 — Overlong replies persisted despite standing instructions

## Diagnosis

### Why a prompt rule was not enough

Opus 5's default replies run long, and lowering effort does not fix it. A conciseness instruction in the prompt decays over a long session — the user had to re-flag verbosity several times in one day despite a standing memory rule. A standing rule also costs context on every request, which the same guidance says makes the model worse.

The harness-level fix is a `Stop` hook: it fires the moment the model decides the turn is done, so the instruction is maximally salient and costs nothing in standing context.

### Check first, re-prompt only on failure

The naive hook re-prompts on every turn, adding a model round-trip each time. Instead a validator inspects the final assistant message. On pass the stop is allowed with zero added cost; on fail the stop is blocked and a corrective instruction names exactly what was missing.

Deterministic checks, no model call: a leading ```orchard-digest fenced block that parses as valid JSON with a non-empty `items` array; each item carrying non-empty `text`, a recognised `kind` and an `importance`; and no emoji anywhere in the reply. Prose-length and validation-opener heuristics are advisory and configurable, since false positives would be worse than the disease.

### Load-bearing safety properties

This sits in the harness/session-lifecycle risk bucket: a buggy `Stop` hook can wedge every turn in a session. Claude Code passes `stop_hook_active` once a hook has already blocked, and the hook must allow the stop when that flag is set, capping corrections at one per turn. Any validator error fails open. The hook is registered in the project's `.claude/settings.json` rather than the user's global settings, and a flag disables it entirely.

[[FEAT-084]] tells the agent the format; this enforces it. Both read the same `responseDigest` enable flag, so a project that turns the digest off is not blocked by the hook.

## Evidence

The fixer's own runs: `verify:feat-085` 58/58, `verify:feat-085-adversarial` 79/79, `verify:feat-085-independent` 60/60, `verify:feat-085-readability` 32/32. The dependency suites also passed alongside it — `verify:feat-083` 22/22 and `verify:feat-083-adversarial` 170/170. Further pass tallies recorded without an adjacent suite name: 27/27, 63/63, 52/52. Typecheck and leak-gate stayed clean. The ticket describes a clean-room pass over the work; no independent dispatch verdict is recorded on the file.

`verify:feat-085-stop-hook` is named in the ticket but no result for it is recorded.

Advisory-only has been the default since 2026-08-15, because the grading of a reply has a known and documented gap. Blocking is opt-in behind `ORCHARD_STOP_HOOK_ENFORCE=1`.

## Implementation notes

Reuse the FEAT-083 digest parser rather than writing a second parser: leading fence only, `text` non-empty. Keep the heuristic checks behind their own switches, separate from the deterministic ones, so a false positive downgrades to noise rather than a blocked turn.

The same principle — cap it deterministically in the harness instead of adding another prompt rule — is already used here by leak-gate, board:check and docs:fresh.

## Verification plan

- A compliant reply (leading valid digest, no emoji) passes and the stop is allowed with no re-prompt. This is the must-FAIL case: a naive always-block implementation re-prompts here.
- A non-compliant reply (no digest, malformed JSON, or emoji present) is blocked with a corrective message naming the specific failure.
- With `stop_hook_active` set, the stop is always allowed.
- A validator that throws or receives garbage input fails open.
- With the disable flag set, the hook is inert.
- Anti-regress: normal sessions still end normally; typecheck and leak-gate clean.

## Migration and rollback

The hook lives in the project's `.claude/settings.json`, so removing the entry removes the behaviour entirely. Short of that, the disable flag makes it inert without touching configuration.

## Risks

A buggy `Stop` hook can wedge every turn in the session. Fail-open and the one-correction-per-turn cap are the properties that prevent this, and they are tested adversarially rather than assumed.

The advisory heuristics are the false-positive surface; they are configurable for that reason.
