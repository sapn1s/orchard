# BUG-026 — /model switch doesn't apply: menu closes, model unchanged on reopen

- **Status:** VERIFIED
- **Area:** claude-station UI + server (model override path)
- **Reported:** 2026-08-05 by user ("switch model to another via /model, there is no acknowledgement, menu just closes and when reopening I have same model — so it's not working")

## Symptom
Opening the `/model` picker and choosing a different model: no acknowledgement/toast, the
menu just closes, and re-opening `/model` shows the ORIGINAL model still selected. The switch
is not persisted and/or not applied to the session.

## Likely area (investigate, don't assume)
- Relates to BUG-021 (which fixed `/model` DISPLAY — resolved model + inherited tag). This is
  the WRITE path: selecting a model must (a) persist the per-session model override and (b)
  apply it to the running/next turn, and (c) reflect back in the picker on reopen.
- Check the `/model` selection handler in `public/app.js` (does it POST the choice?), the
  server route that sets a session/project model override (`src/server/index.ts` + registry
  settings), and whether the runtime/agent-bridge actually receives the new model. Confirm
  whether the failure is: no POST fired, POST fired but not persisted, persisted but not
  applied to the session, or applied but the picker re-reads a stale value.

## Verification (REQUIRED, user-observable)
Real browser: open `/model`, pick a different model → it is acknowledged, persists across a
reopen of the picker AND a page reload, and actually governs the session (the resolved model
the session reports changes). Must FAIL on current code. Add a verify + a Playwright spec.
