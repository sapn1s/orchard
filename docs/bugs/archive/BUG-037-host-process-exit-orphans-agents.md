# BUG-037 — what ends the orchestrator's host CLI mid-flight? (agents orphaned, cause unknown)

- **Status:** DONE 2026-08-12 — RESOLVED via successors: the phenomenon (false turn-end death
  records; the socket/restart kill actor it misframed as "host deaths") is fixed & shipped; its one
  end-to-end anti-regression is carried as a NAMED residual in BUG-043 / BUG-044 (not silently
  closed). The title is wrong and kept for history: no host CLI ever exited.
- **Area:** ~~session host lifecycle / reconnect path~~ → turn-end agent settling / outcome ledger
- **Reported:** 2026-08-09 by user ("what process exit though? it might be a bug depending what
  caused it — I left/rejoined the conversation but that should not cause it")

## Observation
Twice in one session, background agents were reported lost with "running when the previous Claude
Code process exited". The orchestrator's own host CLI had been REPLACED (measured: the live
`claude --resume=<sdk-id>` process was 81s old at check time, while the session had been going for
much longer). Detach/reattach is supposed to be socket-only and safe (BUG-018).

## Evidence collected (and its limits — do NOT over-read it)
SOLID:
- Live CLI pid age 81s; a prior host had ended without any service restart at that time
  (`claude-station` unit last restarted 17:09; the loss happened later).
- No OOM kills in the user journal for the window.
- TWO `claude-station-host-*.scope` units live simultaneously at check time.
CONFOUNDED — must be controlled for in any investigation:
- "25 host scopes ended in the last hour" is NOT evidence of a bug on its own: the verification
  harnesses (verify-restart-survives, verify-zombie-busy, verify-mcp-attach, health-survivor,
  stale-agent-cards…) each spawn REAL sessions with survival enabled, creating real host scopes.
  Agent-driven test traffic is the likely bulk of that count.

## Hypotheses to test (none confirmed)
1. **Reconnect spawns a second CLI and drops the first.** BUG-022 fixed a resume race with a guard
   consulting `survivingHostForSdkSession()`; the zombie-`busy` state (BUG-033) may defeat or
   invert that guard, so a reattach concludes "no live bridge" and starts a fresh `--resume`,
   ending the old CLI (and its IN-PROCESS subagents) — matching the symptom exactly.
2. **Usage/session limit** ends the CLI (one earlier loss this session WAS a stated session limit).
   Distinguish by the CLI's own exit reason/stderr, not by inference.
3. **A verification run collides with the live session** (same dataDir / same sdk session id /
   reaping another host). Check whether any harness can touch a non-scratch host.
4. Broker (`session-host.mjs`) drain/backstop ending a still-wanted child.

## Investigate (ground truth, controlled)
- Instrument/collect: for each host end, WHO ended it (broker log, scope stop reason, CLI exit
  code/stderr tail from `<key>.err`), and whether a NEW host for the same sdk session started
  within seconds (the smoking gun for hypothesis 1).
- Reproduce deliberately with NO verify traffic running: open the dashboard, detach (close tab /
  navigate away) while a turn is in flight, reattach, and observe host pids across the sequence.
  Repeat with a dispatched subagent running to see whether it survives.
- Only after the cause is named: fix, and make the loss HONEST in the UI (an orphaned-agent event
  the user can see — pairs with BUG-030/BUG-033/BUG-034's liveness work).

## Verification
Reproduce the cause deterministically; fix; prove the sequence no longer ends the host (N repeats,
this is intermittent); prove a genuinely finished host still exits (no leak). Anti-regressions:
restart-survives, restart-reconnect-race, resume-refusal, detach, health-survivor.
