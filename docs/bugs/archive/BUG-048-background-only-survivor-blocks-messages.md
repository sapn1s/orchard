# BUG-048 — after a restart, a background-only survivor blocks new messages for its whole drain

- **Status:** OPEN — explore finished; recommend a staged fix, first step just an honest "held, auto-delivers" status.
- **Area:** restart re-adopt path (survival → live bridge), BUG-022/BUG-044 adjacent
- **Reported:** 2026-08-11 by user:
  > "even if something was running, it was likely subagent? not main agent which shouldnt block msgs?"

## In plain terms
After the server restarts while a background helper is still finishing up, sending a new message gets held for
up to about a minute — because the only safe way to handle that leftover helper today is to wait for it to
fully finish. During normal (non-restart) operation the same situation accepts messages immediately, so this
delay only happens right after a restart. The message isn't lost, just slow — an occasional, temporary delay
that contradicts the correct expectation that background helpers shouldn't block new messages. Annoying, not
harmful.

**Recommendation** (investigation, including live tests, is done): a two-stage plan that shortens the wait
without ever risking corrupting the conversation record — one riskier alternative was ruled out on direct
evidence. The safe first step is simply to show honest status ("held by a background task, will auto-deliver in
N seconds") instead of a silent wait; the second stage actually delivers the message into the surviving helper.

**What I need from you:** approve proceeding with that first step (the honest-status surface)?

**If you do nothing:** nothing breaks; messages still arrive (just delayed right after a restart) and this
stays open.

> Everything below is the detailed engineering: the gap timeline, the two-writers hazard, the candidate
> directions, and the live-probe evidence behind the recommendation — reference, skip unless you're building it.

## The gap (confirmed timeline from the live incident)
Restart landed AFTER the session's foreground turn had ended; only a background subagent was
alive. BUG-044's drain correctly HELD (refusing to kill the background work) — but a held drain
also refuses new messages (BUG-033 gate), so the user's message queued ~1min (BUG-045 made that
queue+self-deliver instead of a locked composer, which is why it merely felt slow, not broken).
During NORMAL operation the same state accepts messages freely: background agents never block
the composer. The restriction exists only because of how re-adopt works.

## Why it blocks today (both rules are individually correct)
1. The surviving CLI is only ever DRAINED to completion post-restart — never re-adopted as the
   live conversation bridge.
2. A second CLI cannot be started on the same transcript while the survivor lives: its background
   tasks still append results to that transcript — two writers = BUG-022's corruption class.
Together: "wait until the survivor fully winds down" is the only safe state today.

## Explore (build nothing yet): candidate directions
a. **Full re-adopt as live bridge:** the broker already proxies the CLI's stdio; teach the new
   server to reattach the survivor as the session's live bridge (new turns route into the
   surviving CLI via the broker). Strongest UX; hardest — protocol/session state re-binding,
   BUG-028/029/033 interplay, both runtimes.
b. **Background-only fast-path:** if the survivor's foreground turn is provably ended (turn-end
   on disk, broker state) and only background work holds the drain, allow the new-CLI resume
   IF transcript writes by the survivor can be excluded or serialized (e.g. background results
   spool to a side file the new CLI's server merges) — needs a real answer to the two-writers
   hazard, not a hope.
c. **Keep the block, shrink the wait + tell the truth:** deliver queued messages into the
   survivor's NEXT natural boundary via the broker stdin (the abandon-net machinery already
   writes control frames), and/or surface drain progress (elapsed, what holds it, expected
   bound) on the BUG-045 chip. Cheapest; does not remove the block.
Explore agent should weigh these (plus any better shape), name the two-writers hazard precisely
(who writes what, when), and recommend with a proof-bar sketch. ARCH-002 applies: lifetimes are
declared, unknown never coerced.

## Verification bar (for whichever lands later)
Real-shape: restart with foreground-idle + live background heartbeat survivor → a new user
message reaches the model without waiting for the background work to finish (or, for option c,
within one boundary), background work still completes, transcript uncorrupted (single-writer
proof), BUG-022/029/033/044/045 suites all stay green.
