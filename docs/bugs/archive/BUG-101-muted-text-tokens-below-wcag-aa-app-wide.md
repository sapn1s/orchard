# BUG-101 — muted text tokens fall below WCAG AA app-wide (3rd instance this session; audit the tokens, don't patch the third case)

- **Status:** VERIFIED — AA contrast fix shipped, visually signed off + guarded; two design Qs await your call.
- **Area:** public/styles.css — the ink/muted colour tokens and every surface that uses them
- **Reported:** 2026-08-14 (found by independent visual reviews)

## In plain terms
This was a fix for faint text across the app — grey text that didn't have enough contrast against its
background to meet accessibility standards. The fix is done, verified, and shipped, an independent reviewer
signed off the rendered result, and an automated check now guards it so it can't quietly come back. What's
left are two small design judgment calls (detail at the very bottom of this ticket); they're refinements, not
bugs, and no recommendation was formed for them on purpose — they're genuinely your taste to set.

**Why it matters:** the contrast problem itself is solved. The two remaining questions are about keeping the
"importance" cues on the response-summary card consistent, so a future change doesn't reintroduce a mismatch.

**The two design questions for you:**
1. Should a single high-importance item that happens to sit inside a low-importance group still show as
   full-strength text (which would put a loud line in the quietest section) — or should its emphasis be capped
   to match its group's rank?
2. In dark mode, the four section labels are intentionally near-equal in strength, so the ranking is carried by
   the coloured edge alone. Keep it that way, or make dark mode use the same clear top-to-bottom ladder as
   light mode (which would let the safeguard test be simplified)?

**If you do nothing:** nothing breaks — the accessibility fix stays in place and verified; these two
refinements just stay open as noted.

## Why this is filed as a CLASS, not a third one-off (§N)
Three separate contrast failures surfaced in one session, each found only because an unbiased reviewer
MEASURED the rendered pixels:
1. **Digest low-importance ink** — light 2.77:1, dark 3.91:1 (fixed in BUG-095: now 4.67 / 4.80).
2. **Digest ref chip (clickable) 4.13:1 light + DECISIONS eyebrow 4.19:1 light** (BUG-098, in progress).
3. **Composer footer `.fine`** — measured `#AEB5B0` on `#FFFFFF` = **2.09:1** light; `#565D58` on
   `#131513` = **2.92:1** dark. AA needs 4.5:1 for body text; even the large-text floor is 3.0:1, so
   BOTH themes fail BOTH thresholds. The reviewer had to magnify 3× to read it confidently.

Patching the third instance would leave the same latent defect everywhere else the muted tokens are used.
Per §N (Nth bug in a class → fix the design), the right move is a token-level audit.

## Wanted
1. **Audit every text colour token** (`--ink`, `--ink-2`, `--ink-3`, `--ink-4`, and any local overrides)
   against the surfaces they are actually rendered on (`--bg`, `--sunken`, cards, chips, rails), in BOTH
   themes. Produce a measured table: token × surface → contrast ratio → pass/fail at 4.5:1 (and note
   where the 3.0:1 large-text allowance legitimately applies, e.g. genuinely large type).
2. **Raise the failing tokens to AA** while PRESERVING the visual hierarchy — the whole point of a muted
   token is that it recedes. Do not flatten the ladder into sameness (this exact tension was handled well
   in BUG-095 by re-cutting steps on perceptual lightness rather than just lifting the bottom).
3. Where a token legitimately must stay very quiet (decorative, non-informational), say so explicitly and
   record why it is exempt — do not silently leave a failing value.

## Also from the footer review (fold in)
- The composer footer's three segments (`Direct` · `full access to this machine` · `2 agents running`)
  are typographically UNDIFFERENTIATED — same size, weight, colour, and an identical `·` separator — so a
  permanent session property and a transient activity count read as one flat run-on string. Give the
  transient count a distinct treatment (lighter tone and/or a different separator) so the structure
  matches the meaning. Truncation order is already correct (the count is sacrificed first, the isolation
  label survives).
- **Untested empty state:** every screenshot so far shows "2 agents running". Nobody has verified the
  ZERO-agents rendering — whether it reads cleanly as `Direct · full access to this machine` or leaves a
  dangling `·` / a dead-looking strip. Capture and check it.

## Verification (§C + visual)
- Automated: a check that computes rendered contrast for the audited token×surface pairs and FAILS below
  threshold — so this cannot silently regress again (must-FAIL: run it against the current values and see
  the three known failures reported).
- Visual: re-render the affected surfaces (footer idle AND with agents running, digest card, chips) in
  both themes; unbiased review of the images, per the standing visual gate.
- Anti-regress: the BUG-095 ladder (light 14.83 / 8.13 / 4.67; dark 13.89 / 8.20 / 4.80) and group>item
  hierarchy must not move; verify:feat-083 + adversarial, verify:ui, verify:bug-100-footer-label,
  typecheck, leak-gate.
- Risk bucket: UI/visual + a11y (low functional risk).

## Note on why these kept slipping through
Every one of these passed its functional suite. DOM assertions cannot see contrast; only measuring the
rendered pixels catches it. That is the visual-review gate working as intended — and the argument for the
automated contrast check above, so the gate does not depend on a reviewer remembering to measure.
