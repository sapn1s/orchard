# FEAT-025 — Attachable Serena (LSP) MCP tier: symbol-level code tools for dispatched agents

- **Status:** VERIFIED — per-project UI toggle (Serena + Playwright) BUILT + wired to the real attach seam + verified end-to-end (see 2026-08-04 build entry). Auto-on-by-size/language heuristic still queued (FEAT-033).
- **Area:** claude-station MCP attach / agent tooling
- **Reported:** 2026-08-04 by user
- **Related:** the existing stealth-browser MCP attach (same slot/pattern); FEAT-021 (memory)

## Goal
Give dispatched fix agents LSP-backed semantic code tools (find_symbol,
find_referencing_symbols, get_symbols_overview, replace_symbol_body,
insert_after/before_symbol, search_for_pattern) via Serena (oraios/serena) exposed
as an MCP server — instead of grep + read-whole-file. Token-efficient + more
precise edits; biggest win on the large long-lived repos (external-project-A, external-project-G).

## Design (productized option)
- Serena is an MCP server → attach it the SAME way claude-station already attaches
  the stealth browser (an attachable MCP per project/session). Make it a selectable,
  **opt-in per project** tier (not every scratch project wants a language server).
- Per-project config: `--context ide-assistant --project <hostPath>`; language
  server auto-selected by project language (TS/JS here).
- Serena's own "project memories" complement our board (FEAT-021): code knowledge
  vs task state — keep both.

## Immediate (this repo, enabled now)
Wire Serena for the claude-station repo so our dev sessions use it (repo-level
`.mcp.json` or station attach). Document exact install + config here so the
productized option reuses it. VERIFY the server actually starts + lists tools —
do not claim "enabled" without evidence.

## Caveats
- Per-language language-server setup = moving parts; opt-in only.
- Current running session won't hot-load a newly-added MCP — takes effect for
  sessions launched after.
