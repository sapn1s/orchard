# FEAT-037 — Multi-provider support: decouple from the Claude SDK (vendor-independence)

- **Status:** DONE — all phases complete, VERIFIED-LIVE (2026-08-05). P1 (AgentRuntime seam) · P2a+P2c (CodexRuntime, schema-validated + real ChatGPT-subscription sessions: round-trip, interrupt, approval allow, resume, model list, attributed provider-error card) · P3 (provider picker wired into real session start, 16/0 + live turn) · **P2b (Orchard-owned transcript capture, 15/0 fixture + live close→reopen→history→resume-with-memory — 2026-08-05 P2b entry)**. Honest remainders (documented, not gated silently): UI fork of a Codex session refused pending ancestor-seeding; FEAT-015 survival deliberately stays off for `persistedTranscript:false` engines; search/rename/tag/delete cover the Claude store only.
- **Area:** architecture / agent runtime
- **Reported:** 2026-08-04 by user (strategic: if Anthropic changes terms/pricing, we shouldn't be locked in or lose our work)

## Why
Orchard's engine is `@anthropic-ai/claude-agent-sdk` — Claude-specific. The dashboard,
board, orchestration, isolation, and methodology are provider-agnostic, but model
EXECUTION is not. Vendor lock-in is a real risk; we want a path to run other providers
(OpenAI/Codex, local, etc.). This ticket is a PLAN + complexity assessment first.

## Questions the plan must answer (decision-grade)
1. **Coupling surface:** exactly where/how deep is the SDK used (map with Serena/ast-grep
   over `src/server/agent-bridge.ts` + all SDK imports). Concentrated or spread?
2. **Generic vs Claude-specific:** session lifecycle (start/send/interrupt/close/detach),
   streaming events (agent-started/text-delta/tool/…), tool-use + approvals/permission
   modes, model selection, cost tracking — which port cleanly, which are Claude idioms?
3. **Abstraction shape:** an `AgentRuntime` interface the bridge depends on + a
   `ClaudeRuntime` adapter (no behaviour change) — and the shape a second adapter takes.
4. **The LIGHTER path:** instead of reimplementing each vendor SDK, treat the runtime as a
   pluggable agent CLI/process (the user already runs BOTH Claude + Codex CLIs in
   external-project-G containers). Could Orchard "run a session via <agent CLI>" and capture
   its stream? Is that lower-effort + good-enough for vendor independence? Compare vs the SDK-adapter route.
5. **Complexity: S / M / L**, with a PHASED plan (e.g. P1 extract interface + Claude adapter,
   no behaviour change; P2 a 2nd adapter as proof; P3 per-project/session runtime picker),
   effort per phase, and the biggest risks (feature parity for approvals/tool-use/cost/isolation).

## Deliverable
Append a decision-grade plan to THIS ticket (coupling map + abstraction + lighter-path
verdict + S/M/L + phases + risks). No code changes in this pass — plan only.
