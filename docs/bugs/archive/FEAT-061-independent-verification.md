# FEAT-061 — generation must not verify itself: an independent, executing, adversarial verifier

- **Status:** DONE — CLOSED 2026-08-12 (status word corrected 2026-08-20: it read VERIFIED, and no
  verdict in this ticket's twenty-round lineage is HOLDS; the closing verdict `b281aae3` is
  contract-VALID and BROKEN on an ADJACENT board-tool defect, filed as BUG-071, which the recorded
  bounded-scope bar rules non-blocking. The closure below stands; only the word that claims an
  independent verdict HELD is withdrawn. See the 2026-08-20 entry at the foot.) Closed
  under the BOUNDED-in-scope + TOLERANT-BOUNDED-in-compliance
  closing bar adopted from FEAT-062's precedent. Hole #19 (env assignments falsely granting
  adversarial distinctness) is now **FIXED + committed (`a05ffa1`)**: command identity is ASYMMETRIC —
  CITATION matching keeps assignments in identity (BUG-066, unchanged), DISTINCTNESS strips ALL
  leading env assignments/`env` tokens first via a new `sameProgram()`/`bareCmd()` helper at BOTH
  guard sites (`validateVerdict` + `composeVerdict`); suite 135 → **138/138** with the
  `env-prefix-disguise` ratchet + its distinct-program negative, and BUG-066's citation cases all
  stay green. The ONE closing set's FIRST contract-VALID clean-room verdict (attempt 1, run
  `b281aae3`, no re-prompt) is **VALID; it HELD FEAT-061's core machinery** (hole #19 not re-broken,
  claims #1–#4/#6 all VALID) and the BROKEN finding it returned is **ADJACENT** — a defect in
  `scripts/board.mjs`'s advisory verification-enforcement WARN (the exemption keys on the last
  activity-log date, not on when VERIFIED was reached), filed as **BUG-071** (board-tool, low, gates
  nothing). Per the matrix (VALID + adjacent-only-ticketed → VERIFIED), the ticket is CLOSED. Twenty
  independent clean-room runs across the lineage; nineteen in-scope holes found and ratcheted; the
  twentieth found nothing in the machinery. See the final 2026-08-12 closing entry.
  (Adjacent, non-blocking: BUG-069 — pre-existing FEAT-062 loop flake; BUG-071 — this run's finding.)
- **Verification-class:** fix
- **Area:** methodology (WA + dispatch classes) + tooling (gatekeeper/dispatch) — how work is PROVEN
- **Reported:** 2026-08-10 by user, quoting a practitioner account: an agent rewrote pagination,
  reported "done and tested", CI passed because the fixture had ONE page; staging died after 100
  records (cursor never advanced) — and the SAME agent reviewed its own PR and approved it. "Having
  the model that wrote the code review its own PR is useless — it's grading its own hallucination
  with the exact same blind spots."

## Honest statement of where we stand (no spin)
- **Our fix agents self-verify.** The agent that writes the fix also writes the verify script, runs
  it, and reports PASS. The orchestrator then commits on that report. That is precisely the
  anti-pattern above.
- Mitigations we DO have, and their limits:
  - **Non-vacuity is mandatory** (the test must FAIL pre-fix). Strong — it kills tests that can't
    discriminate — but the FIXER still authors the fixture, so a blind spot that shapes the fixture
    (one page of data) survives it.
  - **Runtime-first verification** (real servers/browsers/sessions, not static reading). Good, but
    still the author's chosen scenarios.
  - **FEAT-050 gatekeeper** — fresh-context, cross-provider, adversarial, fail-closed. This IS the
    decoupled verifier, but it gates PUSHES, is opt-in per repo, and **was never armed on this
    repo**. Arm it.
- Session evidence: every catch so far came from an agent refuting the ORCHESTRATOR's diagnosis
  (BUG-024 misdiagnosis; BUG-033 "point 2 as written would have been a regression"). Nothing has
  ever refuted a FIXER's own work. That asymmetry is the hole.

## Design
1. **New dispatch class `verify` (extends FEAT-060), structurally decoupled:**
   - Fresh context. Given ONLY: the ticket's requirement + the diff + how to run things.
     **NOT given the fixer's report, rationale, or self-assessment** — that narrative is exactly
     what transmits the blind spot.
   - Adversarial objective: "attempt to BREAK this claim", never "check this work" ("double-check"
     invites agreement).
   - **Cross-provider by default** (decorrelated blind spots; also the best use of the scarce GPT
     side per the corrected ROUTING budget).
2. **Executed-evidence requirement (answers "how do you force it to actually run things"):**
   a verdict is INVALID — not a pass — unless it contains:
   - the fixer's own test run, with real output pasted; AND
   - at least ONE adversarial case the fixer's fixture does not cover (boundary/empty/second
     page/concurrent/failure injection), with the command and real output; AND
   - an explicit statement of what it could NOT test and why.
   Static-only review is rejected mechanically (no runtime evidence block ⇒ invalid).
3. **Enforcement, not etiquette:** the orchestrator may not mark a ticket VERIFIED or commit a fix
   on the fixer's word alone for anything above `trivial`. Mechanically: arm the FEAT-050
   pre-push hook on this repo, and make the ticket template require a `Verified-by:` line naming
   an independent agent (fixer id ≠ verifier id).
4. **Scope discipline:** for `trivial`/docs-only work this is overhead — state the threshold
   explicitly rather than applying it everywhere and eroding it by exception.

## Open questions (decide during build, state the answer)
- Does the verifier get the fixer's TEST SCRIPT? (Yes — it must run it; the contamination risk is
  the fixer's PROSE, not their code.)
- Cost: one extra dispatch per non-trivial fix. Acceptable? Recommend yes for `fix`+ classes,
  skip for `trivial`.

## Verification (of this feature itself)
A planted defect whose fixer-authored fixture passes but which fails on an uncovered input:
the `verify` dispatch must catch it (must FAIL to catch it before this ticket = the control).
A static-only verdict is rejected as invalid. The pre-push hook is armed and demonstrably blocks.
Ticket template carries `Verified-by:` and board:check stays green.
