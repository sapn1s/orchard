# FEAT-090 — answer a ticket where you read it, and have that hand the work back to an agent

- **Status:** OPEN — built (steps 1-7 landed); pending unbiased visual review of the Decide card + narrow sheet
- **Area:** the ticket detail view (public/app.js), src/server/board.ts (`appendAnswer`), the board snapshot injected into sessions, and the Needs-You rail
- **Reported:** 2026-08-15 by user

## In plain terms

You read a ticket, you decide, and then there is nowhere to say so. To reply you would have to leave the
ticket, come back to the chat, and re-explain the context I already have. The reply box that does exist
lives on a different screen entirely, and answering there only ticks the item off a list — nothing picks
the work up.

**Why it matters:** Deciding is the part only you can do. Right now the moment you have decided is the
moment the tool stops helping, and the cost lands on you as re-explaining.

**What I need from you:** Nothing yet — a design comes first. One question worth your view when it lands:
after you answer, should work start immediately, or should the answer sit as "ready to proceed" until you
say go?

**If you do nothing:** Answering keeps meaning a trip back to chat.

## What exists today (verified)

- `board.appendAnswer` (board.ts:384) writes the answer into the ticket, append-only, stamped
  `via Needs-You rail` (ANSWER_MARK, board.ts:123).
- Route: `POST /api/projects/:id/board/answer` (index.ts:981). Client: `public/lib/question.js`.
- Answering marks the ticket as resolved so it drops off the rail (board.ts:182-186, :233).

Three gaps:
1. **Answering is only possible from the rail**, not from the ticket you are actually reading.
2. **Answering hands off to nobody.** It records and de-lists. No status change, no agent pickup.
3. **There is no "answered, work can start" state** — a ticket is either waiting on a human or not.

## Wanted

### The handoff (the important half)
Answering a ticket should move it into a state meaning *the human is done; an agent should proceed*, and
that state should reach an agent without the user carrying the context by hand.

Delivery options to weigh in the design pass:
- The **board snapshot already injected into every session at launch** could carry an "answered, awaiting
  action" section — costs nothing new and surfaces at the next turn.
- A **live notification** into an active session, for the case where a session is already open.
- A **sweep** that picks up answered tickets.
These are not exclusive; the snapshot is the cheapest and the most robust to a session not being open.

The answer must remain append-only in the ticket, with its author and time, so the decision record survives.

### The reply affordance (the UI half)
The natural next actions after reading a decision are *reply* or *read more*. Today only "read more" is
possible, and the input is at the far end of a long document.

- **Desktop:** there is unused horizontal space. A side panel holding the reply — visible while reading —
  is the obvious candidate.
- **Small screens:** unsolved and needs actual design, not a shrunk desktop layout. A persistent action
  bar, or a reply control adjacent to the decision itself, are starting points.
- The reply should be reachable *at the decision*, not only after scrolling the whole ticket. Note this
  composes with the ordering law recorded in FEAT-087: the recommendation now appears in the status line,
  so the decision point is at the very top — the reply affordance should be near it.

## Design questions to settle before building
1. Does answering start work immediately, or park it as ready-to-proceed pending a go?
2. What does the answer look like when the decision was multiple-choice — free text, a choice, or both?
3. Should an answered ticket leave the Needs-You rail immediately, or move to a short-lived "handed over"
   state so the user can see it was picked up?
4. Small-screen layout: this needs a real answer, not a fallback.

## How we'll know it works
- Reading a ticket and answering it never requires leaving the ticket, and never requires re-stating context
  that is already in the ticket.
- After answering, an agent can act on it without the user pasting anything (must-FAIL: today nothing picks
  it up).
- The answer is preserved in the ticket, append-only, attributed and timed.
- The reply control is reachable from the decision point on both a wide and a narrow screen — and this is a
  UI change, so it gets the visual review gate: screenshots at both widths, judged by someone who did not
  build it.
