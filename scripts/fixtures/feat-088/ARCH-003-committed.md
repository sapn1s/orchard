...
abandoned. Three different fixes have been written for this; each one passed its own author's tests, and each
was then rejected by an independent check for a case the author had not thought of.
...
- **A — Keep tweaking the guess.** Carry on repairing the current approach, case by case.
  *Cost:* it has already failed three times and still depends on an internal detail of the engine we have
...
leave it: nothing is live, and a rollback adds churn ahead of a decision that replaces both versions anyway.

## How we got here

Three fixes, three rejections, each for a different missed case.
...
That is six distinct ways to be wrong in three attempts, in opposite directions, which is why this is being
raised as a design question rather than filed as a fourth patch. The information the code needs — is the thing
...
fixing more fabrication cases. Nothing is deployed (a restart is required), so no user is affected yet.
**Options:** leave HEAD as-is pending the decision, or revert to attempt 2 (fabricated deaths are louder and
less dangerous than swallowed ones). My recommendation: leave it — nothing is live, and reverting adds churn
